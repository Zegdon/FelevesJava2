...
public class NewWorker extends JDialog {
	...
	btnBeszur.addActionListener(new ActionListener() {
		public void actionPerformed(ActionEvent e) {
			...
			if (verify.validInteger(id, "Azonosító"))
				if (verify.filled(nev, "Név"))
				if (verify.validDate(szid, "Születési idõ"))
						if(verify.filled(munk, "Munkakör"))
							if (verify.validInteger(fiz, "Fizetés")) {
			...
			databaseMethod.insertdata(fieldCheck(id), fieldCheck(nev), fieldCheck(szid), fieldCheck(munk), fieldCheck(fiz));
	...
	public String fieldCheck(JTextField goodField) {
		return goodField.getText();
	}
}
